  ...
  const renderPage = () => {
    switch (currentPage) {
      ...
      case 'ai-scenario': // Both map to same page, logic inside page handles tab default if needed (future improvement)
        return <AIExplanationPage onNavigate={setCurrentPage} />;
      case 'games':
        return <MiniGamesPage onNavigate={setCurrentPage} />;
      case 'test':
        return <TestInterface onNavigate={setCurrentPage} />;
      case 'simulation':
        return <SimulationInterface onNavigate={setCurrentPage} />;
      case 'results':
        return <ResultsPage onNavigate={setCurrentPage} />;
      case 'profile':
        return <ProfilePage onNavigate={setCurrentPage} />;
      case 'analyze-photo':
        return <PhotoAnalysisPage onNavigate={setCurrentPage} />;
      default:
         ...
         <AnimatePresence>
            {isListening && (
                ...
                >
                    ...
                    <p className="text-xs text-slate-300">"Какой штраф за проезд на красный?"</p>
                </motion.div>
            )}
         </AnimatePresence>
         <Button 
